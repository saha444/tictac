...
const MotionButton: FC<Props> = ({ label, classes, onClick }) => {
  return (
    <button
      onClick={onClick}
      className={cn(
        'bg-background group relative h-auto w-56 cursor-pointer rounded-full border-[none] p-1 outline-none',
        classes
      )}
    >
      <span
      ...
      <span className="button-text text-foreground group-hover:text-background absolute top-2/4 left-2/4 ml-4 -translate-x-2/4 -translate-y-2/4 text-center text-lg font-medium tracking-tight whitespace-nowrap duration-500">
        {label}
      </span>
    </button>
  );
